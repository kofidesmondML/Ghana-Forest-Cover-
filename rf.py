...
def combined_loss(weight_mse=0.5, weight_mae=0.5):
    def loss(y_true, y_pred):
        mse_loss = K.mean(K.square(y_true - y_pred))
        mae_loss = K.mean(K.abs(y_true - y_pred))
        return weight_mse * mse_loss + weight_mae * mae_loss
    return loss
...
model.compile(optimizer='adam', loss=combined_loss(weight_mse=0.5, weight_mae=0.5))
